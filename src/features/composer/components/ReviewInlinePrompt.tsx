...
const PresetStep = memo(function PresetStep({
  ...
  return (
    <div className="review-inline-section">
      ...
      >
        <span className="review-inline-option-title">{t("review.reviewAgainstBaseBranch")}</span>
        <span className="review-inline-option-subtitle">{t("review.prStyle")}</span>
      </button>
      <button
        type="button"
        ...
        disabled={isSubmitting}
      >
        <span className="review-inline-option-title">{t("review.reviewUncommittedChanges")}</span>
      </button>
      <button
        type="button"
        ...
        disabled={isSubmitting}
      >
        <span className="review-inline-option-title">{t("review.reviewCommit")}</span>
      </button>
      <button
        type="button"
        ...
        disabled={isSubmitting}
      >
        <span className="review-inline-option-title">{t("review.customReviewInstructions")}</span>
      </button>
    </div>
  );
});
...
const BaseBranchStep = memo(function BaseBranchStep({
  ...
  return (
    <div className="review-inline-section">
      ...
      <div className="review-inline-hint">{t("review.pickRecentBranch")}</div>
      <div className="review-inline-list" role="listbox" aria-label={t("git.branches")}>
        {reviewPrompt.isLoadingBranches ? (
          <div className="review-inline-empty">{t("review.loadingBranches")}</div>
        ) : branches.length === 0 ? (
          <div className="review-inline-empty">{t("review.noBranchesFound")}</div>
        ) : (
...
const CommitStep = memo(function CommitStep({
  ...
  return (
    <div className="review-inline-section">
      ...
      <div className="review-inline-hint">{t("review.selectRecentCommit")}</div>
      <div className="review-inline-list" role="listbox" aria-label={t("git.commits")}>
        {reviewPrompt.isLoadingCommits ? (
          <div className="review-inline-empty">{t("review.loadingCommits")}</div>
        ) : commits.length === 0 ? (
          <div className="review-inline-empty">{t("review.noCommitsFound")}</div>
        ) : (
...
export const ReviewInlinePrompt = memo(function ReviewInlinePrompt({
  ...
  const title = useMemo(() => {
    switch (step) {
      case "baseBranch":
        return t("review.selectBaseBranch");
      case "commit":
        return t("review.selectCommitToReview");
      case "custom":
        return t("review.customReviewInstructions");
      case "preset":
      default:
        return t("review.selectPreset");
    }
  }, [step, t]);
